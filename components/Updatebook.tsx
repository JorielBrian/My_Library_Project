    ...
    return(
        <main className="w-5/6 mx-auto mt-10 mb-20 p-5 bg-orange-400 border-8 border-orange-700 rounded-lg">
            ...
            <form action='' onSubmit={updatebook} className="flex flex-col">
                ...
                <select name="book" className="w-full sm:w-96 px-4 py-1 text-black border-4 border-orange-700 rounded-md" onChange={(e) => handleSelect(e.target.value)} required>
                ...
                <input className="w-full sm:w-96 px-4 m-1 text-black border-4 border-orange-700 rounded-lg" type="text" placeholder="Author" name="author" value={author} onChange={(e) => setAuthor(e.target.value)} required/>
                <label className="w-full font-bold" htmlFor="isbn">ISBN:</label>
                <input className="w-full sm:w-96 px-4 m-1 text-black border-4 border-orange-700 rounded-lg" type="text" placeholder="Isbn" name="isbn" value={isbn} onChange={(e) => setIsbn(e.target.value)} required/>
                ...
                <input className="w-full sm:w-96 px-4 m-1 text-black border-4 border-orange-700 rounded-lg" type="number" placeholder="Quantity" name="quantity" value={quantity} onChange={(e) => setQuantity(Number(e.target.value))} required/>
                <label className="w-full font-bold" htmlFor="borrow">Borrow:</label>
                <input className="w-full sm:w-96 px-4 m-1 text-black border-4 border-orange-700 rounded-lg" type="number" placeholder="Borrow" name="borrow" value={borrow} onChange={(e) => setBorrow(Number(e.target.value))} required/>
                <input type="submit" value="Update book now" className="w-full sm:w-96 mx-auto px-4 py-1 bg-orange-500 border-4 border-orange-700 rounded-full"/>
            </form>
        </main>
    );
}